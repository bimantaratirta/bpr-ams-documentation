...
14. [Scheduled Jobs (Cron)](#14-scheduled-jobs-cron)
...
### 14.1 Auto-Generate Alpha Records

Sistem menjalankan **scheduled job harian** untuk menandai karyawan yang tidak hadir dan tidak memiliki izin sebagai **Alpha**.
...
| **Retry**    | 3x dengan interval 5 menit jika gagal   |
| **Scope**    | Hanya hari kerja (Senin–Sabtu)          |
| **Target**   | Semua karyawan dengan `isActive = true` |

#### Logic Flow

```
┌─────────────────────────────────────────────────────────┐
│  CRON: generate-alpha-records                           │
│  Schedule: 0 23 * * 1-6 (Senin–Sabtu jam 23:00 WIB)   │
└─────────────┬───────────────────────────────────────────┘
              │
              ▼
   ┌──────────────────────┐
   ...
   └──────────┬───────────┘
              ...
              ▼
   ┌──────────────────────────────────┐
   │ 2. Ambil semua Employee          │
   │    WHERE isActive = true         │
   └──────────┬───────────────────────┘
              │
              ▼
   ┌──────────────────────────────────────────────┐
   │ 3. Untuk setiap employee, cek:               │
   │                                              │
   │    a. Ada Attendance record hari ini?         │
   │       → YA: SKIP (sudah check-in)            │
   │                                              │
   │    b. Ada LeaveRequest approved hari ini?     │
   │       → YA: SKIP (sudah izin)                │
   │                                              │
   │    c. Tidak ada keduanya?                     │
   │       → INSERT Attendance + PointRecord       │
   └──────────┬───────────────────────────────────┘
              │
              ▼
   ┌──────────────────────────────────────────────┐
...
```json
{
  "id": "auto_clx_20260214_2023003",
  "date": "2026-02-14",
  "employeeId": "emp_2023003",
  "status": "ALPHA",
  "checkInTime": null,
  "checkOutTime": null,
  "checkInPhoto": null,
  "checkInLat": null,
  "checkInLng": null,
  "checkInInsideRadius": null,
  "isAutoGenerated": true,
  "createdAt": "2026-02-14T23:00:00+07:00"
}
```
...
| Skenario                                     | Handling                                                                              |
| -------------------------------------------- | ------------------------------------------------------------------------------------- |
| Cron gagal jalan                             | Retry 3x. Jika tetap gagal, kirim alert ke admin via notifikasi                       |
| Karyawan check-in telat malam (misal lembur) | Attendance record sudah ada → SKIP, tidak jadi Alpha                                  |
| Karyawan baru ditambahkan hari itu           | Tetap dicek — jika tidak ada check-in & izin → Alpha                                  |
| Karyawan di-nonaktifkan (`isActive = false`) | SKIP — tidak di-generate Alpha                                                        |
| Hari libur nasional                          | Perlu integrasi kalender libur. Jika belum ada, admin bisa manual delete Alpha record |
| Cron jalan double (duplicate)                | Gunakan unique constraint `(employeeId, date)` di Attendance → prevent duplicate      |

#### Monitoring Endpoint (Internal)

```
GET /internal/cron/alpha-status?date=2026-02-14
```
...
```json
{
  ...
  "data": {
    "date": "2026-02-14",
    "executedAt": "2026-02-14T23:00:02+07:00",
    "totalActiveEmployees": 248,
    "alreadyCheckedIn": 231,
    "approvedLeave": 5,
    "alphaGenerated": 12,
    "status": "completed"
  }
